...
class Utils():
    ...
    def __init__(self, ROS_rate, start_time):
        ...
        self._rate_selecteded = ROS_rate
        self.rate = rospy.Rate(self._rate_selecteded) #setup for sleeping in hz
        self._seq = 0
    ...
    def _update_avg_speed(self):
        ...
        if(curr_time.to_sec() > rospy.Duration(.5).to_sec()):
            ...
            if(timeDiff > 0.0): #Update only if we're using a new pose; also, avoid divide by zero
                ...
                self.average_speed = self.average_speed * (1-10/self._rate_selecteded) + speedDiff * (10/self._rate_selecteded)
        ...
        while (state < 100) and (not rospy.is_shutdown()):
            ...
            if (state == 0): 
                ...
                if(not self._force_cap_check()):
                    ...
                    collision_confidence = collision_confidence + 1/self._rate_selecteded
                    ...
                    #TODO: Fix math by maybe .95 * self._rate_selecteded? It should produce a concave up curve when not choosing 0.01.
                    collision_confidence = np.max( np.array([collision_confidence * 95/self._rate_selecteded, .01]))
                ...
                if( self._vectorRegionCompare_symmetrical(self.average_speed, [2/1000,2/1000,3/1000]) 
                    ...
                    collision_confidence = collision_confidence + .5/self._rate_selecteded
                    ...
                    collision_confidence = np.max( np.array([collision_confidence * 90/self._rate_selecteded, .01]))
