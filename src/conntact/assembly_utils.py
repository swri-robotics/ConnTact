...
def euGetMagnitude(euler):
    ...
    class MovePolicy:
        ...
        #MovePolicy init
        def __init__(self, move_mode : str,
                     vector = None,
                     origin = None,
                     force_cmd = [0,0,0],
                     torque_cmd = [0,0,0]):
            self.conntext = None #Reference allows us to get current position when we want.
        ...
        def validate_vec(self, value):
            ...
            """
            if len(value) != 3:
                raise TypeError("Wrong number of elements in 3-d vector passed to MovePolicy!")
